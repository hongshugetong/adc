...
void AT_GET_TID_VERSION_SIZE(uint8_t*OTA_VERSION,uint32_t*FIlelen)
{
    ...
    uint8_t* tid_end=strstr(str,",\"size\":");
    uint8_t* size_start=strstr(str,"\"size\":")+strlen("\"size\":");
...
{ 
    ...
    HAL_UART_Transmit(&huart1,"EC200已经启动\r\n", 20, 1000);
    AT_ATE0();
    osDelay(pdMS_TO_TICKS(500));
    PQ_Read(&message,str,&i);
    AT_BreakMQTT();
    ...
    HAL_UART_Transmit(&huart1,"SIM卡已准备好\r\n", 21, 1000);
    osDelay(pdMS_TO_TICKS(500));
    AT_GSM();
    osDelay(pdMS_TO_TICKS(500));
