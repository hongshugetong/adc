...
int W25QXX_Read(uint8_t* buffer, uint32_t start_addr, uint16_t nbytes)
{
    ...
    addr_bytes[0] = (start_addr >> 16);
    addr_bytes[1] = (start_addr >> 8);
    addr_bytes[2] = (start_addr >> 0);
...
void W25QXX_Erase_Sector(uint32_t sector_addr)
{
    ...
    uint8_t addr_bytes[3];
    addr_bytes[0] = (sector_addr * 4096) >> 16;  // 高字节
    addr_bytes[1] = (sector_addr * 4096) >> 8;
    addr_bytes[2] = (sector_addr * 4096) >> 0;   sector_addr *= 4096;    //每个块有16个扇区，每个扇区的大小是4KB，需要换算为实际地址
...
{
    uint8_t cmd = BLOCK_ERASE_64K_CMD;
    uint8_t addr_bytes[3];
    addr_bytes[0] = (block_addr *65536) >> 16;  // 高字节
    addr_bytes[1] = (block_addr *65536) >> 8;
    addr_bytes[2] = (block_addr *65536) >>0 ;     // 每个块的大小是64KB，需要换算为实际地址

    W25QXX_Wait_Busy(); 
    W25QXX_Write_Enable();  // 擦除操作需要开启写使能
...
void W25QXX_Page_Program(uint8_t* dat, uint32_t WriteAddr, uint16_t nbytes)
{
    ...
    addr_bytes[0] = (WriteAddr *256) >> 16;  // 高字节
    addr_bytes[1] = (WriteAddr *256) >> 8;
    addr_bytes[2] = (WriteAddr *256) >>0 ;          // 低字节
...
void W25Q64_READ_OTA_INFO(OTA_INFO *flag)
{
    ...
    W25QXX_Read((uint8_t*)flag, OTA_INFO_ADDR, sizeof(*flag));
}
    
void W25Q64_WRITE_OTA_INFO(OTA_INFO *flag)
{
    W25QXX_Erase_Sector(0);
    W25QXX_Page_Program((uint8_t*)flag, OTA_INFO_ADDR, sizeof(*flag));
}